...
public:
    ...
    std::vector<iDynTree::Vector6> leftMixedAcceleration, rightMixedAcceleration;
    ...
    bool interpolateFoot(double dT, const std::vector<size_t>& phaseShift, const std::vector<StepPhase> &stepPhase, const FootPrint &foot, std::vector<iDynTree::Transform> &output, std::vector<iDynTree::Twist> &outputTwistsInMixedRepresentation, std::vector<iDynTree::Vector6> &outputAccelerationInMixedRepresentation) {
        ...
        for (size_t phase = 1; phase < phaseShift.size(); ++phase){ //the first value is useless
            ...
            if (stepPhase[instant] == StepPhase::Swing){
                ...
                while (instant < endOfPhase){
                    ...
                    iDynTree::toEigen(rightTrivializedAngAcceleration) = iDynTree::toEigen(RPYRightTrivializedSecondDerivative(0.0, pitchAngle, yawAngle,
                    ...
                    iDynTree::toEigen(outputAccelerationInMixedRepresentation[instant]).block(0, 0, 3, 1) = iDynTree::toEigen(linearAcceleration);
                    iDynTree::toEigen(outputAccelerationInMixedRepresentation[instant]).block(3, 0, 3, 1) = iDynTree::toEigen(rightTrivializedAngAcceleration);
                    ++instant;
                }
...
void FeetCubicSplineGenerator::getFeetAccelerationInMixedRepresentation(std::vector<iDynTree::Vector6> &lFootAccelerationInMixedRepresentation, std::vector<iDynTree::Vector6> &rFootAccelerationInMixedRepresentation) const
{
    std::lock_guard<std::mutex> guard(m_pimpl->mutex);
